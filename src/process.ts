  ...
  try {
    ...
    for (let iter = 1; iter <= MAX_ITER; iter++) {
      ...
      const systemPrompt = [
        ...
        "- IMAGENS REAIS (RÁPIDO — usa SEMPRE o modo LOTE): planeia TODAS as imagens que a página precisa (hero, cards de destinos, galeria, propriedade…), escreve um `imagens.json` = [{\"prompt\":\"<inglês, detalhado, cinematográfico>\",\"out\":\"public/images/<nome>.webp\",\"aspect\":\"16:9\"}, …], e gera-as TODAS DE UMA VEZ com `node scripts/studio-image.mjs --batch imagens.json`. Corre em paralelo (~segundos para 15 imagens) em vez de uma a uma. Só usa o modo de imagem única para 1 imagem avulsa. Usa-as com <img src=\"/images/...\"> ou next/image. NUNCA placeholder.com, via.placeholder, unsplash aleatório, nem divs de cor sólida onde devia haver foto.",
