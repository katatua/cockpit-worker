  ...
  try {
    ...
    for (let iter = 1; iter <= MAX_ITER; iter++) {
      ...
      const systemPrompt = [
        ...
        "- IMAGENS REAIS (usa o modo LOTE e sê FOCADO — a conta de imagens tem rate limit, por isso gera um conjunto ESSENCIAL de 5 a 8 imagens de alto impacto, não 15+; reutiliza a mesma imagem em cards repetidos quando faz sentido). Planeia as imagens-chave (hero + 3-4 destinos + 1-2 de destaque), escreve um `imagens.json` = [{\"prompt\":\"<inglês, detalhado, cinematográfico>\",\"out\":\"public/images/<nome>.webp\",\"aspect\":\"16:9\"}, …], e gera-as com `node scripts/studio-image.mjs --batch imagens.json` (uma só chamada). Usa-as com <img src=\"/images/...\"> ou next/image. NUNCA placeholder.com, via.placeholder, unsplash aleatório, nem divs de cor sólida onde devia haver foto.",
